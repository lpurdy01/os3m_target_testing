...
def handle_data(data_packet):
    ...
    for i in range(0, len(data_packet), 4):
        lower = int.from_bytes(data_packet[i:i+2], byteorder='little', signed=False)
        upper = int.from_bytes(data_packet[i+2:i+4], byteorder='little', signed=False)
        full_value = (upper << 16) | lower
        print(f"Lower: {lower}, Upper: {upper}, Combined: {full_value}")  # Debug print
